<script setup lang="ts">
  ...
  const sql = computed(() => questions.questions.map((item: typeof questions.questions, index: number) => {
    return `INSERT INTO questions(question_id, question, options, correct_option) VALUES ('${Number(index)+Number(questionIdFrom.value)}','${item.title}','${JSON.stringify(item.options)}','${item.correct}');
